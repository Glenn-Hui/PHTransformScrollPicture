...
public class PHTimer {
    ...
    public typealias PHTimerHandler = (PHTimer) -> Void
    
    private var handler: PHTimerHandler
    
    public init(interval: DispatchTimeInterval, repeats: Bool = false, leeway: DispatchTimeInterval = .seconds(0), queue: DispatchQueue = .main , handler: @escaping PHTimerHandler) {
    ...
    public static func repeaticTimer(interval: DispatchTimeInterval, leeway: DispatchTimeInterval = .seconds(0), queue: DispatchQueue = .main , handler: @escaping PHTimerHandler ) -> PHTimer {
        return PHTimer(interval: interval, repeats: true, leeway: leeway, queue: queue, handler: handler)
    }
    ...
    public func rescheduleHandler(handler: @escaping PHTimerHandler) {
        self.handler = handler
        internalTimer.setEventHandler { [weak self] in
            if let strongSelf = self {
                handler(strongSelf)
...
public class PHCountDownTimer {
    ...
    private let handler: (PHCountDownTimer, _ leftTimes: Int) -> Void
    
    public init(interval: DispatchTimeInterval, times: Int,queue: DispatchQueue = .main , handler:  @escaping (PHCountDownTimer, _ leftTimes: Int) -> Void ) {
        
        self.leftTimes = times
        self.originalTimes = times
        self.handler = handler
        self.internalTimer = PHTimer.repeaticTimer(interval: interval, queue: queue, handler: { _ in
        })
...
private extension DispatchTimeInterval {
    ...
    private static func fromSeconds(_ seconds: Double) -> DispatchTimeInterval {
